...
export const test: Test = async ({ FileSystem, Workspace, Extension, SideBar, Locator, expect }) => {
  ...
  await expect(rows.nth(10)).toHaveText('Global')
  await expect(rows.nth(11)).toHaveText('Call Stack')
  await expect(rows.nth(11)).toHaveAttribute('aria-posinset', '4')
  await expect(rows.nth(11)).toHaveAttribute('aria-setsize', '4')
  await expect(rows.nth(12)).toHaveText('processTimersunknown:493')
}
